...
abstract class DFA<T> extends TreeNode<T> {
  public ArrayList<Scope<? extends hasChild_Scope<?>>> decls(Ref<? extends hasChild_Ref<?>> r,
      Scope<? extends hasChild_Scope<?>> s) {
    return null;
  }
}
...
class dfaState<T extends hasChild_DFAState<T>>
  ...
  public ArrayList<Scope<? extends hasChild_Scope<?>>> decls(Ref<? extends hasChild_Ref<?>> r,
    ...
    ArrayList<Scope<? extends hasChild_Scope<?>>> varRes = new ArrayList<Scope<? extends hasChild_Scope<?>>>();
    for (Scope<? extends hasChild_Scope<?>> sVar : s.var()) {
      if (scopeTrace)
        System.out.println("In resolving " + r.pp() + " in state " + this.pp() + ", traversing a VAR " + "edge from "
            + s.pp() + " to " + sVar.pp() + ", and traversing DFA to state " + this.varT().pp());
      varRes.addAll(this.varT().decls(r, sVar));
    }

    ArrayList<Scope<? extends hasChild_Scope<?>>> modRes = new ArrayList<Scope<? extends hasChild_Scope<?>>>();
    for (Scope<? extends hasChild_Scope<?>> sMod : s.mod()) {
      if (scopeTrace)
        System.out.println("In resolving " + r.pp() + " in state " + this.pp() + ", traversing a MOD " + "edge from "
            + s.pp() + " to " + sMod.pp() + ", and traversing DFA to state " + this.modT().pp());
      modRes.addAll(this.modT().decls(r, sMod));
    }

    ArrayList<Scope<? extends hasChild_Scope<?>>> impRes = new ArrayList<Scope<? extends hasChild_Scope<?>>>();
    for (Scope<? extends hasChild_Scope<?>> sImp : s.imp()) {
      if (scopeTrace)
        System.out.println("In resolving " + r.pp() + " in state " + this.pp() + ", traversing a IMP " + "edge from "
            + s.pp() + " to " + sImp.pp() + ", and traversing DFA to state " + this.impT().pp());
      impRes.addAll(this.impT().decls(r, sImp));
    }

    ArrayList<Scope<? extends hasChild_Scope<?>>> lexRes = new ArrayList<Scope<? extends hasChild_Scope<?>>>();
    for (Scope<? extends hasChild_Scope<?>> sLex : s.lex()) {
      if (scopeTrace)
        System.out.println("In resolving " + r.pp() + " in state " + this.pp() + ", traversing a LEX " + "edge from "
            + s.pp() + " to " + sLex.pp() + ", and traversing DFA to state " + this.lexT().pp());
      lexRes.addAll(this.lexT().decls(r, sLex));
    }

    return (!varRes.isEmpty()) ? varRes : (!modRes.isEmpty()) ? modRes : (!impRes.isEmpty()) ? impRes : lexRes;
...
class dfaStateFinal<T extends hasChild_DFAState<T>>
  ...
  public ArrayList<Scope<? extends hasChild_Scope<?>>> decls(Ref<? extends hasChild_Ref<?>> r,
    ...
    ArrayList<Scope<? extends hasChild_Scope<?>>> res = new ArrayList<Scope<? extends hasChild_Scope<?>>>();
